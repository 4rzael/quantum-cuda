...
namespace client
{
    ...
    namespace calculator_grammar
    {
        ...
        x3::rule<class expression_class, ast::program> const expression("expression");
        x3::rule<class term_class, ast::program> const term("term");
        x3::rule<class factor_class, ast::operand> const factor("factor");

        auto expression_def = (term
            >> *(   (char_('+') >> term)
                |   (char_('-') >> term)
            ));

        auto term_def = (factor
                >> *(   (char_('*') >> factor)
                    |   (char_('/') >> factor)
                    ));

        auto factor_def = (uint_
            |   '(' >> expression >> ')'
            |   (char_('-') >> factor)
            |   (char_('+') >> factor));
